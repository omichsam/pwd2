...
$stats = [
  ...
  'checked' => getCount("SELECT COUNT(*) FROM assessments WHERE hospital_id = $hospital_id AND 'pending'"),
              ...
              <div class="stat-card">
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <div class="h4 mb-0"><?= $stats['total'] - $stats['pending'] - $stats['rejected']   ?></div>
                  </div>
                </div>
              </div>
            </div>
    ...
    const disabilityChart = new Chart(ctx, {
      ...
      data: {
        labels: [<?= implode(',', array_map(function ($item) {
          return "'" . htmlspecialchars($item['disability_type']) . "'";
        }, $disability_stats)) ?>],
        datasets: [{
